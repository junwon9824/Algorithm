...
#                 /100*(datediff(h.END_DATE ,h.START_DATE))+1 )   as FEE
 ...
 round( (c.DAILY_FEE) 
    * (case 
    ...
    when datediff(END_DATE,START_DATE)+1 < 30 then 95
    when DATEDIFF(END_DATE,START_DATE)+1 < 90 then 92
     when DATEDIFF(END_DATE,START_DATE)+1 >= 90 then  85
       end) /100 * (DATEDIFF(h.END_DATE,h.START_DATE)+1))as FEE
