...
public class Solution {
    ...
    public int solution(int k, int n, int[][] reqs) {
        ...
        // 타입마다 시간을 분리할 리스트
         List<Time>[] customertime = new ArrayList[k + 1];

        for (int i = 0; i < k + 1; i++) {
            customertime[i] = new ArrayList<>();
        }
        ...
        for (int[] req : reqs) {
            ...
            customertime[type].add(new Time(startTime, startTime + duration)); //문제에서 주어진
            // 정보...
        }
        ...
        for (int type = 1; type < k + 1; type++) {
            ...
            for (int counselors = 1; counselors <= (n - k) + 1; counselors++) {
                ...
                int waitTime = calculationTime(customertime[type], counselors);
