...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		while (T-- > 0) {
			...
			String rd = br.readLine(); // 문제에서 r, d에 해당하는 명령어
	...
	public static void solve(String rd, Deque<Integer> deque) {
		...
		printall(deque, isRight);

	}

	public static void printall(Deque<Integer> deque, boolean isRight) {
		...
		if (deque.size() > 0) { // 만약 출력 할 원소가 한 개 이상일 경우
			...
			if (isRight) { // 정방향일경우
				...
				sb.append(deque.pollFirst()); // 먼저 첫 번째 원소를 넘겨준다. 이걸 생략하면 처음수의 앞에 , 오게된다.
				...
				sb.append(deque.pollLast()); // 먼저 뒤에서부터 첫 번째 원소를 넘겨준다.이걸 생략하면 처음수의 앞에 , 오게된다.
